...
\end{align}
where $X_{i}$ is defined same as before and we assume it depends on parameter $\theta$. Objective function \eqref{eq:long} in most cases is not known analytically. In such cases it makes difficult for the search procedure to update the search parameter with out estimating the cost over infinite long trajectory. Another subclassifications of optimization problems is based on feasible region $\Theta$, see \eqref{eq:opt}.
